...
# ==============================================================================
# 🔒 GÜVENLİK: API ANAHTARI (Secrets'tan okur)
# ==============================================================================
try:
...
# ---------------------------------------------------------
# 🛑 GİZLİLİK AYARLARI (GitHub Şeridini Gizle)
# ---------------------------------------------------------
st.markdown("""
    <style>
    ...
    /* GÜVEN SKORU KUTUSU (Analiz Sonucu İçin) */
...
# Forum Verileri (Eski Konular Geri Geldi)
    ...
    st.session_state['forum_konulari'] = [
        {
            ...
            "icerik": "NASA'nın son yayınladığı fotoğraflarda görülen kemik benzeri yapılar aslında sadece rüzgar erozyonu ile oluşmuş kayalardır. Pareidolia etkisi nedeniyle beynimiz bunları kemiğe benzetiyor.",
...
def internette_ara(sorgu):
    try:
        with DDGS() as ddgs:
            return list(ddgs.text(sorgu, region='tr-tr', max_results=5))
    except Exception:
        return []
...
def teyit_et(iddia, veriler, api_key, ton):
    ...
    KRİTİK KURALLAR:
    1. 'Plants vs Zombies', oyun veya reklamları GÖRMEZDEN GEL.
    2. Sonuçlar yetersizse genel bilgini kullan ama bunu belirt.
    3. Cevabın en başına mutlaka "GÜVEN ORANI: %XX" yaz (0-100 arası).
...
with tab1:
    ...
    with col_input:
        sorgu = st.text_area("Haber/İddia:", height=120, placeholder="Örn: Mars'ta su bulundu mu?")
    with col_opts:
        st.write("🗣️ **Konuşma Şekli**")
        
        # SEÇENEKLERİ DURUMA GÖRE GİZLEME (Kilit Efekti)
        if st.session_state['premium_uye']:
            secenekler = ["Ciddi", "Eğlenceli", "Samimi", "Ağzı Bozuk (Argo)"]
        else:
            # Normal üyeler kilitli isimleri görür
    ...
    if st.button("Gerçeği Bul", type="primary", use_container_width=True):
        ...
        if not kullanilacak_key or "BURAYA" in kullanilacak_key:
            ...
            with st.spinner(f"Aranıyor..."):
                ...
                if not res: st.warning("⚠️ Güncel veri bulunamadı, genel bilgi kullanılıyor.")
                ...
                with st.expander("Bulunan Kaynaklar"):
                    st.write(res)
